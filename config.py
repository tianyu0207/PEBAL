...
if os.getenv('volna') is not None:
    ...
    C.volna = '/home/yu/anomaly_segment/'

"""please config ROOT_dir and user when u first using"""
C.repo_name = 'anomaly_segment'
...
C.img_root_folder = '/home/yu/yu_ssd/city_scape'
C.gt_root_folder = "/home/yu/yu_ssd/city_scape"
C.pretrained_model = C.volna + 'DATA/resnet-weight/wider_resnet38.pth.tar'
...
"""Image Config"""
C.num_classes = 19 + 1 # for anomaly detection
C.background = -1
C.image_mean = np.array([0.485, 0.456, 0.406])  # 0.485, 0.456, 0.406
C.image_std = np.array([0.229, 0.224, 0.225])

C.image_height = 800  # 896  # 712
C.image_width = 800  # 896  # 712
...
if os.getenv('learning_rate'):
    ...
    # C.lr = 0.04  meta-ood=1e-5
    C.lr = 1e-5

# small-batch for 2080Ti-debug
...
C.nepochs = 15
C.max_samples = C.num_train_imgs  # max(C.num_train_imgs, C.num_unsup_imgs)
C.cold_start = 0
