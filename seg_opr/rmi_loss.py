...
def convert_to_onehot(gt, num_classes):

    gt[gt==255] = num_classes        # handle 255 case
    b, h, w = gt.shape
    target_one_hot = torch.FloatTensor(b*h*w, num_classes+1).cuda()
    target_one_hot.zero_()
    target_one_hot.scatter_(1, gt.float().view(-1, 1).long(), 1)
    target_one_hot = target_one_hot.view(b, h, w, num_classes+1)

    return target_one_hot[:, :, :, :-1]
...
class RMILoss(nn.Module):
	...
	"""
	def __init__(self, use_sigmoid=False, num_classes=0, rmi_radius=3, rmi_pool_way=0, rmi_pool_size=3,
    rmi_pool_stride=3, loss_weight_lambda=0.5, loss_weight=1, lambda_way=1):
	...
	def forward_sigmoid(self, logits_4D, labels_4D):
		...
		# valid label
		# valid_onehot_labels_4D = convert_to_onehot(labels_4D.long() * label_mask_3D.long(), num_classes=self.num_classes).float()
		...
		valid_onehot_labels_4D = convert_to_onehot(labels_4D.long(), num_classes=self.num_classes).float()
